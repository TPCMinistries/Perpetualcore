...
export function RecordingList({ projects }: { projects: PressProject[] }) {
  ...
  return (
    <section aria-labelledby="recordings-title">
      ...
      {visibleProjects.length > 0 && <div className="hidden overflow-hidden border border-black/10 bg-white md:block">
        <Table>
          <TableHeader>
            ...
            {visibleProjects.map((project) => (
              <TableRow key={project.id} className="group">
                <TableCell>
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <Link href={`/press/studio/recordings/${project.id}`} className="font-semibold text-zinc-950 hover:text-[#1648d8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1648d8] focus-visible:ring-offset-2">
                        {project.title}
                      </Link>
                ...
                <TableCell className="text-right">
                  <Link href={`/press/studio/recordings/${project.id}`} className="inline-flex min-h-11 items-center px-3 text-sm font-semibold text-[#1648d8] hover:text-[#1039ad] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1648d8]" aria-label={`Open ${project.title}`}>
      ...
      {visibleProjects.length > 0 && <div className="grid gap-3 md:hidden">
        {visibleProjects.map((project) => (
          <Link
            key={project.id}
            href={`/press/studio/recordings/${project.id}`}
            className="border border-black/10 bg-white p-4 transition-colors duration-200 hover:border-[#1648d8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1648d8] focus-visible:ring-offset-2"
          >
