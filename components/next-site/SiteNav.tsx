...
export function SiteNav() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-5">
    ...
    <AnimatePresence>{open ? <motion.nav initial={reduce?false:{opacity:0,y:-8,scale:.985,filter:"blur(4px)"}} animate={{opacity:1,y:0,scale:1,filter:"blur(0px)"}} exit={{opacity:0,y:-5,scale:.99,filter:"blur(3px)"}} transition={{type:"spring",duration:.24,bounce:0}} className="mx-auto mt-2 max-w-[1500px] origin-top rounded-[18px] border border-black/10 bg-[#f3f1ea] p-3 shadow-xl lg:hidden">{links.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="flex min-h-12 items-center rounded-xl px-4 text-base font-bold hover:bg-black/[.05]">{label}</Link>)}<Link href="/packages" className="mt-2 flex min-h-12 items-center justify-center rounded-xl bg-[#2457ff] font-bold text-white">Ways to start</Link></motion.nav> : null}</AnimatePresence>
  </header>;
}
