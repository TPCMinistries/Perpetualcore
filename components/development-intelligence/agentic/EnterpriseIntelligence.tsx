...
export function EnterpriseIntelligence() {
  ...
  return <div className="space-y-8">
    <section className="overflow-hidden rounded-[28px] border border-primary/20 bg-[#f5f3ff] px-6 py-8 sm:px-10 sm:py-11">
      <Badge className="border border-primary/20 bg-white text-primary hover:bg-white"><BrainCircuit className="mr-1.5 h-3.5 w-3.5" />Enterprise Development Intelligence</Badge>
      ...
      <div className="mt-7 rounded-2xl border border-primary/20 bg-white p-5 sm:p-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2">{suggested.map((item) => <button key={item} type="button" onClick={() => setQuestion(item)} className="min-h-10 cursor-pointer rounded-full border border-primary/20 bg-primary/10 px-3 py-2 text-left text-xs font-medium text-primary transition-colors hover:bg-primary/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">{item}</button>)}</div>
    ...
    ].map((item) => <Card key={item.title} className="border-slate-200 shadow-none"><CardContent className="p-6"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><item.icon className="h-5 w-5" /></div><h2 className="mt-4 font-semibold text-slate-950">{item.title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{item.detail}</p></CardContent></Card>)}</section>}
