...
export function PlaybookStudio() {
  ...
  return <div className="space-y-8">
    <section className="overflow-hidden rounded-[28px] border border-primary/20 bg-[#f5f3ff] p-6 sm:p-9">
      <div className="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end"><div><Badge className="border border-primary/20 bg-white text-primary hover:bg-white"><BookOpenCheck className="mr-1.5 h-3.5 w-3.5" />Agent Playbook Studio</Badge><h1 className="mt-4 max-w-3xl text-3xl font-semibold tracking-[-0.03em] text-[#1e1b4b] sm:text-4xl">Turn your best analysis method into a governed, reusable playbook.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-slate-700">Version goals, specialist teams, evidence standards, and exclusions. Activation is explicit, and every update creates a reviewable version.</p></div><Button size="lg" onClick={newPlaybook} className="min-h-12 bg-primary hover:bg-primary"><Plus className="mr-2 h-4 w-4" />New playbook</Button></div>
    </section>
    ...
    <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
      <aside aria-label="Playbook versions" className="space-y-3">
        <div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-primary">Library</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Playbooks</h2></div>{loading && <Loader2 className="h-5 w-5 animate-spin text-primary" />}</div>
        {!loading && playbooks.length === 0 ? <Card className="border-dashed border-primary/20 bg-primary/40 shadow-none"><CardContent className="p-6 text-center"><BookOpenCheck className="mx-auto h-7 w-7 text-primary" /><h3 className="mt-3 font-semibold text-slate-950">No playbooks yet</h3><p className="mt-2 text-sm leading-6 text-slate-600">Create a governed draft for a question your organization asks repeatedly.</p></CardContent></Card> : playbooks.map((playbook) => <button key={playbook.id} type="button" onClick={() => openPlaybook(playbook)} className={`w-full cursor-pointer rounded-xl border p-4 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${draft.id === playbook.id ? "border-primary/20 bg-primary/10" : "border-slate-200 bg-white hover:border-primary/20"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-semibold text-slate-950">{playbook.name}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-600">{playbook.purpose}</p></div><Badge variant="outline" className={playbook.status === "active" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "bg-slate-50 text-slate-700"}>{playbook.status}</Badge></div><div className="mt-3 flex items-center justify-between text-xs text-slate-500"><span>Version {playbook.version}</span><span>{new Date(playbook.updatedAt).toLocaleDateString()}</span></div></button>)}
      </aside>

      <section aria-label="Playbook editor" className="space-y-5">
        <Card className="border-slate-200 shadow-none"><CardContent className="space-y-5 p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-primary">{draft.id ? `Version ${draft.version}` : "New draft"}</p><h2 className="mt-1 text-2xl font-semibold text-slate-950">Playbook definition</h2></div>{draft.id && <Badge variant="outline" className={draft.status === "active" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "bg-slate-50"}>{draft.status === "active" ? <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> : <Clock3 className="mr-1 h-3.5 w-3.5" />}{draft.status}</Badge>}</div>
        ...
        {!plan ? <Card className="border-dashed border-primary/20 bg-primary/40 shadow-none"><CardContent className="flex flex-col items-center p-10 text-center"><Sparkles className="h-7 w-7 text-primary" /><h3 className="mt-3 font-semibold text-slate-950">Generate the plan before saving.</h3><p className="mt-2 max-w-lg text-sm leading-6 text-slate-600">The agent will create a rubric, select specialists, require evidence, and state exclusions for you to inspect.</p></CardContent></Card> : <>
          <div className="grid gap-4 lg:grid-cols-2"><Card className="border-slate-200 shadow-none"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wide text-primary">Specialist team</p><div className="mt-3 flex flex-wrap gap-2">{strings(plan.recommendedSpecialists).map((item) => <Badge key={item} variant="outline" className="bg-primary/10 text-primary">{item.replaceAll("_", " ")}</Badge>)}</div></CardContent></Card><Card className="border-slate-200 shadow-none"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Evidence requirements</p><ul className="mt-3 space-y-2">{strings(plan.evidenceRequirements).map((item) => <li key={item} className="flex items-start gap-2 text-sm leading-5 text-slate-700"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />{item}</li>)}</ul></CardContent></Card></div>
          <Card className="border-slate-200 shadow-none"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wide text-primary">Versioned rubric</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{rubric.map((item, index) => { const row = item as Record<string, unknown>; return <div key={String(row.key ?? index)} className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-semibold text-slate-950">{String(row.label ?? `Criterion ${index + 1}`)}</h3><Badge variant="outline">Weight {String(row.weight ?? 1)}</Badge></div><p className="mt-2 text-sm leading-6 text-slate-600">{String(row.question ?? "")}</p><p className="mt-3 text-xs leading-5 text-primary">Evidence: {String(row.evidenceRequirement ?? "Exact source evidence required")}</p></div>; })}</div></CardContent></Card>
