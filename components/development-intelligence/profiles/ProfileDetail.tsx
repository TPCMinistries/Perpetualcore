...
export function ProfileDetail({ subjectId }: { subjectId: string }) {
  ...
  if (loading) return <div className="space-y-8 pb-12"><DevelopmentNav /><div className="flex min-h-[50vh] items-center justify-center gap-3 text-sm text-slate-600"><Loader2 className="h-5 w-5 animate-spin text-primary" />Loading development profile…</div></div>;

  if (loadError || !profile) {
    return (
      <div className="space-y-6 pb-12">
        <DevelopmentNav />
        <Link href="/dashboard/development/profiles" className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-slate-600 hover:text-primary"><ArrowLeft className="h-4 w-4" />Development profiles</Link>
    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Dialog open={linkOpen} onOpenChange={setLinkOpen}>
              <DialogTrigger asChild><Button size="lg" disabled={!consentActive} className="min-h-12 bg-primary hover:bg-primary"><Link2 className="mr-2 h-4 w-4" />Link conversation</Button></DialogTrigger>
              <DialogContent className="sm:max-w-lg">
                <form onSubmit={linkSession}>
                  ...
                  <div className="space-y-4 py-6">
                    ...
                    <div className="rounded-xl border border-primary/20 bg-primary/10 p-3 text-xs leading-5 text-primary"><ShieldCheck className="mr-1 inline h-4 w-4" />Linking assigns only evidence already attributed to the matching participant label. It does not re-analyze identity.</div>
                  </div>
                  <DialogFooter><Button type="button" variant="outline" onClick={() => setLinkOpen(false)} disabled={linking}>Cancel</Button><Button type="submit" disabled={linking || !sessionId.trim() || !participantLabel.trim()} className="bg-primary hover:bg-primary">{linking ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Link2 className="mr-2 h-4 w-4" />}Link conversation</Button></DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
          </div>
      ...
      <section>
        ...
        {trends.length === 0 ? <Card className="border-dashed border-primary/20 bg-primary/40 shadow-none"><CardContent className="flex flex-col items-center px-6 py-10 text-center"><CircleDashed className="h-7 w-7 text-primary" /><h3 className="mt-3 font-semibold text-slate-950">A trajectory appears after a conversation is linked.</h3><p className="mt-2 max-w-lg text-sm leading-6 text-slate-600">Link an authorized report with participant-attributed evidence to begin this profile&apos;s baseline.</p></CardContent></Card> : <div className="grid gap-4 lg:grid-cols-2">{trends.map((trend) => {
          ...
          return <Card key={trend.key} className="border-slate-200 shadow-none"><CardContent className="p-6"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{trend.count} {trend.count === 1 ? "observation" : "observations"}</p><h3 className="mt-2 font-semibold text-slate-950">{trend.label}</h3></div><Badge variant="outline" className={direction.style}><direction.icon className="mr-1 h-3.5 w-3.5" />{direction.label}</Badge></div><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-primary/10 px-3 py-1 text-primary">Current: {readable(trend.latest.evidenceLevel)}</span>{trend.previous && <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-700">Previous: {readable(trend.previous.evidenceLevel)}</span>}</div>{trend.latest.developmentalAction && <div className="mt-4 rounded-xl bg-primary/10 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-primary">Developmental action</p><p className="mt-1 text-sm leading-6 text-primary">{trend.latest.developmentalAction}</p></div>}<div className="mt-4 flex items-center justify-between text-xs text-slate-500"><span>{new Date(trend.latest.observedAt).toLocaleDateString()}</span>{trend.latest.analysisId && <Link href={`/dashboard/development/analyses/${trend.latest.analysisId}`} className="inline-flex min-h-9 items-center font-medium text-primary hover:underline">Inspect source report <ExternalLink className="ml-1 h-3.5 w-3.5" /></Link>}</div></CardContent></Card>;
        })}</div>}
      </section>

      <section>
        <div className="mb-5"><p className="text-sm font-semibold text-primary">Commitments</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">Follow-through from the evidence</h2></div>
        {profile.commitments.length === 0 ? <Card className="border-dashed border-slate-200 shadow-none"><CardContent className="px-6 py-10 text-center"><ClipboardCheck className="mx-auto h-7 w-7 text-slate-400" /><h3 className="mt-3 font-semibold text-slate-950">No commitments are linked yet.</h3><p className="mt-2 text-sm text-slate-600">Commitments attributed in linked reports will appear here for human follow-up.</p></CardContent></Card> : <div className="space-y-3">{profile.commitments.map((commitment) => <Card key={commitment.id} className="border-slate-200 shadow-none"><CardContent className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className={commitment.status === "completed" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}>{readable(commitment.status)}</Badge>{commitment.dueDate && <span className="inline-flex items-center text-xs text-slate-500"><CalendarDays className="mr-1 h-3.5 w-3.5" />Due {new Date(`${commitment.dueDate}T12:00:00`).toLocaleDateString()}</span>}</div><p className="mt-3 font-medium text-slate-950">{commitment.statement}</p><p className="mt-2 text-xs text-slate-500">{commitment.ownerLabel ? `Owner: ${commitment.ownerLabel}` : "Owner not verified"} · <Link href={`/dashboard/development/analyses/${commitment.analysisId}`} className="font-medium text-primary hover:underline">Inspect source report</Link></p></div><div className="flex items-center gap-2"><Label htmlFor={`commitment-${commitment.id}`} className="sr-only">Status for {commitment.statement}</Label><Select value={commitment.status} disabled={updatingCommitment === commitment.id} onValueChange={(value) => requestCommitmentUpdate(commitment, value as ProfileCommitment["status"])}><SelectTrigger id={`commitment-${commitment.id}`} className="w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">Open</SelectItem><SelectItem value="completed">Completed</SelectItem><SelectItem value="changed">Changed</SelectItem><SelectItem value="cancelled">Cancelled</SelectItem><SelectItem value="unverified">Unverified</SelectItem></SelectContent></Select>{updatingCommitment === commitment.id && <Loader2 className="h-4 w-4 animate-spin text-primary" aria-label="Updating commitment" />}</div></CardContent></Card>)}</div>}
      </section>
      ...
      <Dialog open={pendingCommitment !== null} onOpenChange={(open) => { if (!open && !updatingCommitment) { setPendingCommitment(null); setCommitmentNote(""); } }}>
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button type="button" disabled={!pendingCommitment || !commitmentNote.trim() || updatingCommitment !== null} className="bg-primary hover:bg-primary" onClick={() => { if (pendingCommitment) void updateCommitment(pendingCommitment.id, pendingCommitment.status, commitmentNote); }}>
