...
export function RunDetail({ runId }: { runId: string }) {
  ...
  if (loading) return <div className="flex min-h-[55vh] items-center justify-center gap-3 text-sm text-slate-600"><Loader2 className="h-5 w-5 animate-spin text-primary" />Loading run evidence…</div>;
  ...
  const statusTone = run.status === "review_ready" ? "border-amber-200 bg-amber-50 text-amber-800" : run.status === "failed" ? "border-rose-200 bg-rose-50 text-rose-800" : "border-primary/20 bg-primary/10 text-primary";
