...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: meetingProfile } = await supabase
      .from("profiles")
      .select("organization_id")
      .eq("id", user.id)
      .single();
    if (meetingProfile?.organization_id) {
      const gate = await gateFeature("calendar_integration", meetingProfile.organization_id);
      if (!gate.allowed) {
