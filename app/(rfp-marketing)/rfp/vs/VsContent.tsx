...
function renderCell(cell: Cell, colIndex: number) {
  ...
  if (cell === "no") {
    return (
      <span className="text-zinc-500" aria-label="No">
        ...
        >
          <table className="w-full min-w-[820px] text-[13px]">
            <caption className="sr-only">
              RFP Engine compared against Instrumentl, Grants.gov, OpenGrants,
              Submittable, direct AI tools, and capture consultants.
            </caption>
            <thead>
              <tr className="border-b border-white/5">
                <th scope="col" className="sticky left-0 z-10 bg-zinc-950 px-4 py-4 text-left font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-400">
                  Capability
                </th>
                {COLUMNS.map((col, i) => (
                  <th
                    ...
                    className={`px-3 py-4 text-center font-mono text-[10px] uppercase tracking-[0.18em] ${
                      i === 0 ? "text-emerald-300" : "text-zinc-400"
                    }`}
                  >
                    {col}
                  </th>
            ...
            <tbody>
              {ROWS.map((row) => (
                ...
                >
                  <th scope="row" className="sticky left-0 z-10 bg-zinc-950/95 px-4 py-4 text-left align-top font-normal">
                    <div className="text-[14px] text-zinc-100">{row.label}</div>
                    {row.detail ? (
                      <div className="mt-1 max-w-md text-[12px] text-zinc-400">
                        {row.detail}
                      </div>
                    ) : null}
                  </th>
