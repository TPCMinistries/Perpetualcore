...
export default function CompaniesPage() {
  return <div className="min-h-screen bg-[#12151c] text-white"><SiteNav /><main id="main-content" className="pt-28">
    <section className="px-5 py-14 sm:px-8 sm:py-20"><div className="mx-auto max-w-[1500px]"><p className="text-[10px] font-black tracking-[.16em] text-[#d7ff3f]">THE COMPANY NETWORK / LIVE FIELD</p><h1 className="mt-7 max-w-5xl text-[clamp(3.75rem,7.6vw,7.5rem)] font-black leading-[.78] tracking-[-.08em]">DEPTH<br/><span className="text-white/25">BEATS</span><br/>BREADTH.</h1><div className="mt-12 grid gap-5 border-t border-white/10 pt-8 md:grid-cols-2"><p className="text-xl font-bold leading-8">Perpetual Core is not a holding page for unrelated ideas.</p><p className="max-w-2xl text-lg leading-8 text-white/50">It is one operating layer that earns the right to produce focused companies where institutional work demands dedicated depth.</p></div></div></section>
    ...
    <section className="bg-[#ff6338] px-5 py-20 text-[#12151c] sm:px-8"><div className="mx-auto flex max-w-[1500px] flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><h2 className="max-w-4xl text-4xl font-black leading-[.9] tracking-[-.06em] sm:text-6xl">WE DO NOT LAUNCH COMPANIES TO FILL A GRID.</h2><Link href="/studio" className="inline-flex min-h-14 shrink-0 items-center justify-center rounded-2xl bg-[#12151c] px-6 font-black text-white">See how a build earns scale</Link></div></section>
  </main><SiteFooter /></div>;
}
