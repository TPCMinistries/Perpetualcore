...
function getHandoffState(account: HandoffAccount) {
  const paid =
    ...
    account.status.toLowerCase().includes("paid");
  const blocked =
    account.paymentStatus === "blocked" || Boolean(account.blockedTaskCount);
  ...
  const hasOverdue = Boolean(
    account.overdueTaskCount && account.overdueTaskCount > 0,
  );
  ...
  if (paid && !hasContext) {
    return {
      ...
      detail:
        "Money or intent is clear. Send the client handoff so the first operating lane has real context.",
      score: 2,
  ...
  if (hasContext && !hasTasks) {
    return {
      ...
      detail:
        "Client context is in. Sync handoff tasks so the first lane has owners and due dates.",
      score: 3,
  ...
  if (hasContext && hasTasks) {
    return {
      ...
      detail:
        "Context and tasks are connected. Open the account room and work the next visible task.",
      score: 5,
  ...
  return {
    ...
    label: "Needs routing",
    detail:
      "Choose the buyer path, then send checkout, invoice, proposal, or handoff based on readiness.",
    score: 0,
  ...
  const rows = accounts.slice(0, 8).map((account, index) => {
    ...
    return [
      ...
      account.clientHandoffPath
        ? `Client handoff: ${account.clientHandoffPath}`
        : "Client handoff: not synced yet",
    ].join("\n");
  });
...
function buildClientHandoffAsk(account: HandoffAccount) {
  const handoffUrl =
    typeof window !== "undefined" && account.clientHandoffPath
      ? `${window.location.origin}${account.clientHandoffPath}`
      : "";
...
export default function HandoffsPage() {
  ...
  async function fetchHandoffs() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Could not load handoff queue",
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return data.activeClients
      ...
      .sort((a, b) => {
        ...
        return (
          new Date(a.nextTaskDueDate || a.createdAt).getTime() -
          new Date(b.nextTaskDueDate || b.createdAt).getTime()
        );
      });
  ...
  const readyPercent =
    data.activeClients.length > 0
      ? Math.round(
          ((counts.ready + counts.needs_tasks) / data.activeClients.length) *
            100,
        )
      : 0;
    ...
    <div className="space-y-6 pb-10">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
              Use this queue to see which accounts need client context, which
              need tasks, and which are ready for the next delivery action.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              ...
              onClick={() =>
                copyText(
                  "Handoff queue brief",
                  buildHandoffBrief(filteredAccounts),
                )
              }
            >
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          [
            ...
            "Send or complete the client handoff.",
          ],
          [
            "Needs tasks",
            counts.needs_tasks,
            "Turn submitted context into kickoff work.",
          ],
          ["Ready", counts.ready, "Open the room and run the next task."],
          [
            "Blocked",
            counts.blocked,
            "Resolve payment, access, or overdue work.",
          ],
            ...
            <div className="bg-slate-950 p-5 text-white">
              ...
              <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-3 text-xs leading-5 text-slate-400">
                  Counts accounts with submitted context or task-backed delivery
                  as moving toward readiness.
                </p>
              </div>
            </div>
              ...
              {[
                {
                  ...
                  detail:
                    "Workflow owner, tools, examples, rules, and success metric.",
                  icon: FileText,
                },
                {
                  title: "2. Create task plan",
                  detail:
                    "Kickoff, context mapping, first deliverable, and expansion review.",
                  icon: Target,
                },
                {
                  title: "3. Operate weekly",
                  detail:
                    "Use the account room to keep AI, owner, work, and proof connected.",
                  icon: ShieldCheck,
                },
              ].map((item) => (
                <div
      ...
      <Card className="rounded-lg shadow-none">
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 rounded-lg border bg-slate-50 p-3 md:grid-cols-[1fr_220px]">
            ...
            <Select
              value={stateFilter}
              onValueChange={(value) => setStateFilter(value as HandoffState)}
            >
                  ...
                  >
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-md border px-2 py-1 text-xs font-medium ${state.className}`}
                        >
                          {state.label}
                        </span>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                        ...
                        <span className="rounded-md bg-slate-100 px-2 py-1">
                          Context:{" "}
                          {account.handoffContextReceived
                            ? "received"
                            : "missing"}
                        </span>
                        <span className="rounded-md bg-slate-100 px-2 py-1">
                        ...
                        <span className="rounded-md bg-slate-100 px-2 py-1">
                          Handoff link:{" "}
                          {account.clientHandoffPath ? "ready" : "needs sync"}
                        </span>
                    ...
                    <div className="grid content-start gap-2">
                      ...
                      {account.clientHandoffPath ? (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              copyText(
                                "Client handoff link",
                                getAbsolutePath(account.clientHandoffPath),
                              )
                            }
                          >
                          ...
                          <Button
                            ...
                            className="h-8 rounded-md"
                          >
                            <Link href={account.clientHandoffPath}>
                              Handoff{" "}
                        ...
                        >
                          <Link
                            href={`/dashboard/proposals${account.leadId ? `?lead=${encodeURIComponent(account.leadId)}` : ""}`}
                          >
                            Sync path{" "}
                            <PackageCheck className="ml-2 h-3.5 w-3.5" />
                          </Link>
                        </Button>
                      )}
                    </div>
      ...
      <Card className="rounded-lg shadow-none">
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Keep the AI flexible across the account, but force it to respect
                sequence.
              </p>
            </div>
        ...
        <CardContent className="grid gap-3 md:grid-cols-4">
          {[
            [
              "Do not start vague delivery",
              "No paid account should expand without context, first lane, and task owner.",
            ],
            [
              "Never lock to one script",
              "Use account state to choose checkout, invoice, procurement, handoff, or delivery next.",
            ],
            [
              "Preserve account memory",
              "Lead notes, payment state, handoff fields, tasks, and follow-ups stay attached.",
            ],
            [
              "Expand from proof",
              "Use the first lane result to justify the next workflow or 90-day operating lane.",
            ],
          ].map(([title, detail]) => (
